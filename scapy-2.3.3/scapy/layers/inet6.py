        ...
        if x is None:
            ...
            if in6_isaddrTeredo(x):   # print Teredo info
                ...
                return "%s [Teredo srv: %s cli: %s:%s]" % (
                    self.i2h(pkt, x), server, maddr, mport)
    ...
    def i2h(self, pkt, x):
        if x is None:
            ...
            if isinstance(dst, Gen):
                r = sorted(map(conf.route6.route, dst))
                if r[0] == r[-1]:
                    x = r[0][1]
                else:
    ...
    def default_payload_class(self, p):
        if self.nh == 58:  # ICMPv6
            ...
            if len(p) >= icmp6typesminhdrlen.get(
                    t, sys.maxsize):  # Other ICMPv6 messages
...
class IPv6(_IPv6GuessPayload, Packet, IPTools):
    ...
    def hashret(self):
        ...
        if conf.checkIPsrc and conf.checkIPaddr and not in6_ismaddr(sd):
            ...
            return strxor(sd, ss) + struct.pack("B", nh) + \
    ...
    def answers(self, other):
        ...
        if conf.checkIPaddr:
            ...
            if in6_ismaddr(other.dst):
                ...
                return False
        if self.nh == 58 and isinstance(
                self.payload, _ICMPv6) and self.payload.type < 128:
...
class _HopByHopOptionsField(PacketListField):
    ...
    def __init__(self, name, default, cls, curpos,
                 count_from=None, length_from=None):
        self.curpos = curpos
...
class TruncPktLenField(PacketLenField):
    ...
    def __init__(self, name, default, cls, cur_shift,
                 length_from=None, shift=0):
...
class DomainNameListField(StrLenField):
    ...
    def __init__(self, name, default, fld=None,
                 length_from=None, padded=False):
        self.padded = padded
...
class NIReplyDataField(StrField):
    ...
    def h2i(self, pkt, x):
        ...
        if qtype == 2:  # DNS name
            ...
            if isinstance(x, list) and x and not isinstance(x[0], int):
                x = [0] + x
            ttl = x[0]
            names = x[1:]
    ...
    def addfield(self, pkt, s, val):
        ...
        if t == 2:
            ...
            return s + "".join(map(lambda x_y1: struct.pack("!I",
                                                            x_y1[0]) + inet_pton(socket.AF_INET6, x_y1[1]), tmp))
        elif t == 4:
            return s + "".join(map(lambda x_y2: struct.pack("!I",
                                                            x_y2[0]) + inet_pton(socket.AF_INET, x_y2[1]), tmp))
        else:
            return s + tmp
...
class L3RawSocket6(L3RawSocket):
    ...
    def __init__(self, type=ETH_P_IPV6, filter=None,
                 iface=None, promisc=None, nofilter=0):
...
class _IPv6inIP(SuperSocket):
    ...
    def __init__(self, family=socket.AF_INET6,
                 type=socket.SOCK_STREAM, proto=0, **args):
