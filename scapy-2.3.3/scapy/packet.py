...
class Packet(BasePacket):
    ...
    def __init__(self, _pkt="", post_transform=None,
                 _internal=0, _underlayer=None, **fields):
        self.time = time.time()
        self.sent_time = None
...
#        backcolor=makecol(0.376, 0.729, 0.525, 1.0)
        ...
        def make_dump_txt(x, y, txt):
            return pyx.text.text(XDSTART + x * XMUL, (YDUMP - y) *
                                 YMUL, r"\tt{%s}" % hexstr(txt), [pyx.text.size.Large])

        def make_box(o):
            return pyx.box.rect(o.left(), o.bottom(), o.width(),
                                o.height(), relcenter=(0.5, 0.5))
    ...
    def _show_or_dump(self, dump=False, indent=3, lvl="",
                      label_lvl="", first_call=True):
        """
...
class NoPayload(Packet):
    ...
    def answers(self, other):
        return isinstance(other, NoPayload) or isinstance(
            other, conf.padding_layer)
