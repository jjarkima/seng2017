...
class Net(Gen):
    ...
    def _parse_net(cls, net):
        ...
        netmask = int(tmp[1])
        return map(lambda x, y: cls._parse_digit(x, y), tmp[0].split(
            "."), map(lambda x, nm=netmask: x - nm, (8, 16, 24, 32))), netmask
    ...
    def __new__(cls, name, bases, dct):
        if "fields_desc" in dct:  # perform resolution of references to other packets
            ...
            for f in current_fld:
                if isinstance(
                        f, Packet_metaclass):  # reference to another fields_desc
