...
class ASN1Tag(EnumElement):
    ...
    def clone(self):  # /!\ not a real deep copy. self.codec is shared
        return self.__class__(self._key, self._value,
                              self._context, self._codec)
...
class ASN1_Object:
    ...
    def __repr__(self):
        return "<%s[%r]>" % (self.__dict__.get(
            "name", self.__class__.__name__), self.val)
...
class ASN1_BIT_STRING(ASN1_Object):
    ...
    def __repr__(self):
        if len(self.val) <= 16:
            return "<%s[%r] (%d unused bit%s)>" % (self.__dict__.get(
                "name", self.__class__.__name__), self.val, self.unused_bits, "s" if self.unused_bits > 1 else "")
        else:
            s = self.val_readable
            if len(s) > 20:
                s = s[:10] + "..." + s[-10:]
            return "<%s[%r] (%d unused bit%s)>" % (self.__dict__.get(
                "name", self.__class__.__name__), s, self.unused_bits, "s" if self.unused_bits > 1 else "")
...
class ASN1_OID(ASN1_Object):
    ...
    def __repr__(self):
        return "<%s[%r]>" % (self.__dict__.get(
            "name", self.__class__.__name__), self.oidname)
