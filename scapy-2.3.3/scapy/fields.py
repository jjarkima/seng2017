...
class Field(object):
    ...
    def getfield(self, pkt, s):
        ...
        return s[self.sz:], self.m2i(
            pkt, struct.unpack(self.fmt, s[:self.sz])[0])
    ...
    def __repr__(self):
        return "<Field (%s).%s>" % (
            ",".join(x.__name__ for x in self.owners), self.name)
    ...
    def randval(self):
        ...
        fmtt = self.fmt[-1]
        if fmtt in "BHIQ":
            return {"B": RandByte, "H": RandShort,
                    "I": RandInt, "Q": RandLong}[fmtt]()
        elif fmtt == "s":
...
class PadField(object):
    ...
    def addfield(self, pkt, s, val):
        ...
        return s + sval + struct.pack("%is" %
                                      (self.padlen(len(sval))), self._padwith)
    ...
    def i2h(self, pkt, x):
        if x is None:
            ...
            if isinstance(dst, Gen):
                r = sorted(map(conf.route.route, dst))
                if r[0] != r[-1]:
...
class StrFixedLenEnumField(StrFixedLenField):
    ...
    def __init__(self, name, default, length=None,
                 enum=None, length_from=None):
...
class NetBIOSNameField(StrFixedLenField):
    ...
    def m2i(self, pkt, x):
        x = x.strip("\x00").strip(" ")
        return "".join(map(lambda x, y: chr(
            (((ord(x) - 1) & 0xf) << 4) + ((ord(y) - 1) & 0xf)), x[::2], x[1::2]))
...
class BoundStrLenField(StrLenField):
    ...
    def __init__(self, name, default, minlen=0,
                 maxlen=255, fld=None, length_from=None):
...
class FieldListField(Field):
    ...
    def __init__(self, name, default, field,
                 length_from=None, count_from=None):
        if default is None:
            default = []  # Create a new list for each instance
        self.field = field
...
class FieldLenField(Field):
    ...
    def __init__(self, name, default, length_of=None, fmt="H",
                 count_of=None, adjust=lambda pkt, x: x, fld=None):
        Field.__init__(self, name, default, fmt)
...
class BitFieldLenField(BitField):
    ...
    def __init__(self, name, default, size, length_of=None,
                 count_of=None, adjust=lambda pkt, x: x):
...
class _EnumField(Field):
    ...
    def i2repr_one(self, pkt, x):
        if self not in conf.noenum and not isinstance(
                x, VolatileValue) and x in self.i2s:
            return self.i2s[x]
        return repr(x)
...
class _IPPrefixFieldBase(Field):
    ...
    def __init__(self, name, default, wordbytes,
                 maxbytes, aton, ntoa, length_from):
