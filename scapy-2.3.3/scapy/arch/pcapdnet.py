...
if conf.use_winpcapy:
        ...
        try:
            ...
            while p:
                if p.contents.name.endswith(iff):
                    ...
                    while a:
                        if hasattr(
                                socket, 'AF_LINK') and a.contents.addr.contents.sa_family == socket.AF_LINK:
                            ap = a.contents.addr
    ...
    class _PcapWrapper_pypcap:
        ...
        def setfilter(self, f):
            ...
            if pcap_compile(self.pcap, byref(self.bpf_program),
                            filter_exp, 0, -1) == -1:
...
if conf.use_pcap and conf.use_dnet:
    class L3dnetSocket(SuperSocket):
        ...
        def __init__(self, type=ETH_P_ALL, filter=None,
                     promisc=None, iface=None, nofilter=0):
            self.iflist = {}
            self.intf = dnet.intf()
            if iface is None:
                iface = conf.iface
