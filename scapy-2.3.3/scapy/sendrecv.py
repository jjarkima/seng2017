...
def sndrcv(pks, pkt, timeout=None, inter=0,
...
def __gen_send(s, x, inter=0, loop=0, count=None, verbose=None,
               realtime=None, return_packets=False, *args, **kargs):
    if isinstance(x, str):
...
def send(x, inter=0, loop=0, count=None, verbose=None,
         realtime=None, return_packets=False, *args, **kargs):
...
@conf.commands.register
def sendpfast(x, pps=None, mbps=None, realtime=None,
              loop=0, file_cache=False, iface=None):
...
@conf.commands.register
def srp(x, iface=None, iface_hint=None, filter=None,
...
def __sr_loop(srfunc, pkts, prn=lambda x: x[1].summary(), prnfail=lambda x: x.summary(
), inter=1, timeout=None, count=None, verbose=None, store=1, *args, **kargs):
    n = 0
    r = 0
    ct = conf.color_theme
    if verbose is None:
...
def sndrcvflood(pks, pkt, prn=lambda s_r: s_r[
                1].summary(), chainCC=0, store=1, unique=0):
    if not isinstance(pkt, Gen):
        pkt = SetGen(pkt)
...
@conf.commands.register
def srpflood(x, filter=None, iface=None, iface_hint=None,
             nofilter=None, *args, **kargs):
